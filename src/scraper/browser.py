...
class BrowserManager:
    ...
    async def restart(self) -> None:
        """Restart the browser after a crash (serialised — only one restart at a time)."""
        async with self._restart_lock:
            # Another coroutine may have already restarted while we waited for the lock.
    ...
    async def new_page(self) -> Page:
        ...
        try:
            ...
            self._context = None   # mark as dead so restart proceeds
            await self.restart()
            page = await self._context.new_page()  # type: ignore[union-attr]
            await self._configure_page(page)
            return page
